...
func parseHTML(body string, wg *sync.WaitGroup, dbChan chan<- Course) []Course {
   ...
   Arguments:
		...
		ctxggg (context.Context): A context that will ensure we stop doing work if an error occurs
...
func Scraper() {
	...
	*/
	...
	// Spawn workers
	parsersWg.Add(1)
	go parseHTML(body, &parsersWg, sendDB)
